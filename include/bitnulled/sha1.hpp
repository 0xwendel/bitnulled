...
#ifndef BITNULLED_SHA1_HPP
#define BITNULLED_SHA1_HPP
...
inline std::array<uint8_t, 20> SHA1::final_bytes()
{
    ...
    {
        bytes[i * 4 + 0] = static_cast<uint8_t>((digest[i] >> 24) & 0xff);
        bytes[i * 4 + 1] = static_cast<uint8_t>((digest[i] >> 16) & 0xff);
        bytes[i * 4 + 2] = static_cast<uint8_t>((digest[i] >> 8) & 0xff);
        bytes[i * 4 + 3] = static_cast<uint8_t>(digest[i] & 0xff);
    }
...
inline std::string SHA1::final_binary()
{
    auto bytes = final_bytes();
...
inline std::string SHA1::final()
{
    ...
    for (uint8_t b : bytes)
    {
        result << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(b);
    }
    return result.str();
}
